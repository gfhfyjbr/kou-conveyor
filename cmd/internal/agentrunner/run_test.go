...
func TestRunMainUsesProviderAuthenticationConfiguration(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			}, func() []string { return nil }, strings.NewReader(`{"prompt":"hello"}`), io.Discard, &stderr, Config{Name: "kou-conveyor-runner", ParseRequest: parseTestRequest, Providers: providers})
			if test.wantError {
...
func TestRunMainUsesLLMConfigurationFromEnvironment(t *testing.T) {
	...
	code := RunMain(
		...
		&stdout,
		&stderr,
		Config{Name: "kou-conveyor-runner", ParseRequest: parseTestRequest, Providers: providers},
	)
	if code != 0 || !selected {
...
func testConfig(client Client) Config {
	return Config{Name: "kou-conveyor-runner", ParseRequest: parseTestRequest, Providers: []Provider{{
