...
from harness_harbor.agent import KouConveyor
...
class BundleTests(unittest.TestCase):
    def test_changed_binary_is_rejected_after_construction(self):
        with TemporaryDirectory() as temporary:
            path = Path(temporary)
            (path / "kou-conveyor-runner").write_bytes(b"original")
            ...
            (path / "kou-conveyor-runner").write_bytes(b"replacement")
    ...
    def test_bad_trajectory_is_not_silently_accepted(self):
        with TemporaryDirectory() as temporary:
            path = Path(temporary)
            (path / "kou-conveyor-runner").write_bytes(b"runner")
            (path / "manifest.json").write_text(
                json.dumps(
                    {
                        "revision": "a" * 40,
    ...
    def test_invalid_model_and_reasoning_fail_before_installation(self):
        for model in ("test", "anthropic/test", "openai/"):
            with ExitStack() as stack:
                ...
                KouConveyor(bundle="missing", logs_dir=Path("."), model_name=model)
