...
class KouConveyor(BaseInstalledAgent):
    SUPPORTS_ATIF = True
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._remote = PurePosixPath("/installed-agent/kou-conveyor")
    ...
    async def run(
        ...
        env = {
            "KOU_CONVEYOR_LLM_PROVIDER": (
                "fireworks" if self._provider == "fireworks_ai" else self._provider
            ),
            "KOU_CONVEYOR_LLM_API_KEY": connection.api_key,
        }
        if connection.configured_base_url:
            env["KOU_CONVEYOR_LLM_BASE_URL"] = connection.configured_base_url
        command = (
